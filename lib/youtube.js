...
// ============================================================
// NOTA: Este archivo fue RECONSTRUIDO. Nunca se vio completo en el
// historial — solo se sabe que subia Shorts via YouTube Data API v3
// y que incluia containsSyntheticMedia: true para declarar IA.
// La logica aqui es la estandar de googleapis; verificar en la primera
// prueba real que el token y el flujo de subida funcionen igual que antes.
// ============================================================

/**
 * Construye un cliente OAuth2 autenticado a partir de:
 *  - youtube-credentials.json (OAuth client "Desktop app" de Google Cloud)
 *  - youtube-token.json (generado una vez con setup-youtube-auth.js; se
 *    auto-renueva solo mientras tenga refresh_token)
 */
function buildAuthClient({ credentialsPath, tokenPath }) {
  ...
  // Persistimos el token renovado cuando googleapis lo refresca solo.
 ...
 * @param {object} params
 * @param {string} params.localPath - archivo de video local
 * @param {string} params.title
 * @param {string} params.description
 * @param {string[]} [params.tags]
 * @param {string} [params.credentialsPath]
...
async function publishShort({ localPath, title, description, tags = [], credentialsPath, tokenPath }) {
  ...
  // El titulo de un Short debe incluir el hashtag #Shorts para que YouTube
  // lo clasifique correctamente en el feed de Shorts.
  ...
  const res = await youtube.videos.insert({
    ...
    requestBody: {
      snippet: {
        ...
        categoryId: '27', // Educacion
      },
      status: {
        privacyStatus: process.env.YOUTUBE_PRIVACY || 'public',
        selfDeclaredMadeForKids: false,
        // Declaracion de contenido alterado/sintetico (IA) — YouTube lo
        // exige y su ausencia SI afecta la distribucion en su plataforma.
