...
const ESTILO_POR_CANAL = {
  curious4d: ', vibrant saturated colors, extreme high contrast, dramatic cinematic lighting, sense of wonder, professional viral thumbnail style, ultra detailed, no text',
  hipotesis4d: ', epic dramatic mood, vibrant saturated colors, extreme high contrast, cinematic lighting, professional viral thumbnail style, ultra detailed, no text',
  oscuro4d: ', terrifying horror atmosphere, extreme high contrast, dramatic red and black tones, harsh cinematic lighting, dark fog, hyperrealistic, professional viral horror thumbnail style, shocking and eye-catching, no text',
};
...
async function generateThumbnail(tituloTexto, profileName, outputDir, promptEscena = null) {
  const width = 1280;
  const height = 720;

  let prompt;
  if (promptEscena) {
    const estilo = ESTILO_POR_CANAL[profileName] || ESTILO_POR_CANAL.curious4d;
    prompt = `YouTube thumbnail background, ${promptEscena}${estilo}`;
  } else {
    prompt = THUMB_PROMPTS[profileName] || THUMB_PROMPTS.curious4d;
  }

  logger.info('Miniatura: generando imagen base con Agnes', { profileName, variable: !!promptEscena });
