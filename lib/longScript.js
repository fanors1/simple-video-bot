...
const { investigarKeywords, elegirMejores } = require('./keywords');
...
Estructura del relato en ${numPuntos + 2} bloques narrativos (cada uno de 3 a 4 frases, hablados por un narrador con voz grave; se conciso, sin relleno, para que el video dure alrededor de 3 minutos):
...
Reglas de los visuales (visual_prompt en ingles), MUY IMPORTANTE:
- Cada visual_prompt debe ilustrar EXACTAMENTE lo que narra ESE bloque, mostrando la progresion de la historia. Si el bloque habla de la familia llegando a la casa, muestra a la familia frente a la casa; si habla de la hija en su cuarto de noche, muestra a la niña en el cuarto oscuro; si habla de una figura en el pasillo, muestra el pasillo con la figura al fondo. Los visuales deben CONTAR la historia, variando segun el momento (los protagonistas, la casa por dentro y por fuera, las habitaciones, la noche, los sucesos).
- Puedes mostrar personas (la familia, la madre, la hija, el hombre) en las escenas, pero SIEMPRE a media distancia o de espaldas o en penumbra, NUNCA rostros humanos en primer plano (la IA los deforma). Las presencias sobrenaturales siempre lejanas, como sombras o siluetas insinuadas.
- Foto-realistas y cinematograficos (photorealistic, cinematic film still, realistic lighting, horror atmosphere).
...
async function generateLongScript({ profile: profileName = 'curious4d', numPuntos = 5, account = null } = {}) {
  ...
  for (let intento = 1; intento <= MAX_INTENTOS; intento++) {
    try {
      ...
      let keywordsReales = [];
      try {
        const investigadas = await investigarKeywords(profileName, parsed.topic || '');
        keywordsReales = elegirMejores(investigadas, 12);
      } catch (err) {
        logger.warn('No se pudieron investigar keywords, se usan las de Agnes', { error: err.message });
      }

      const tagsCombinados = [...new Set([...(parsed.tags || []), ...keywordsReales])];
      const tags = buildTagsLargo(parsed.topic || 'recopilacion', tagsCombinados, profileName);

      logger.info('Guion largo generado', { topic: parsed.topic, puntos: puntos.length, keywordsReales: keywordsReales.length, intento });
      return { topic: parsed.topic || `Recopilacion de ${profileName}`, descripcion: (parsed.descripcion || '').trim(), tags, keywordsReales, intro, puntos, cierre };
    } catch (err) {
      ultimoError = err;
