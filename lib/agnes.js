...
async function generateVideo(theme, { outputDir = './output', rawPrompt = null, horizontal = false } = {}) {
  ...
  const framesDeseado = Number(process.env.AGNES_NUM_FRAMES) || 121;
  const framesSeguro = 121;
  const framesAProbar = framesDeseado !== framesSeguro ? [framesDeseado, framesSeguro] : [framesSeguro];

  let ultimoError;
  for (const numFrames of framesAProbar) {
    const MAX_INTENTOS_VIDEO = 3;
    for (let intento = 1; intento <= MAX_INTENTOS_VIDEO; intento++) {
      try {
        return await intentarGenerarVideo(prompt, outputDir, horizontal, numFrames);
      } catch (err) {
        ultimoError = err;
        const esRechazoFrames = /400/.test(err.message || '');
        if (esRechazoFrames && numFrames !== framesSeguro && intento === 1) {
          logger.warn(`Agnes rechazo ${numFrames} frames, bajando a ${framesSeguro} (valor seguro)`, { error: err.message });
          break;
        }
...
async function intentarGenerarVideo(prompt, outputDir, horizontal = false, numFrames = 121) {
  logger.info('Creando tarea de video en Agnes AI', { prompt, horizontal, numFrames });
