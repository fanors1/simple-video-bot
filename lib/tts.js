...
function corregirCitasBiblicas(texto) {
  return texto.replace(/(\d+)\s*:\s*(\d+)(?:\s*[-–]\s*(\d+))?/g, (match, cap, versIni, versFin) => {
    if (versFin) return `capitulo ${cap} versiculos ${versIni} al ${versFin}`;
    return `capitulo ${cap} versiculo ${versIni}`;
  });
}
...
async function textToSpeech(text, outputPath, options = {}) {
  ...
  const textoNarrar = options.corregirCitas ? corregirCitasBiblicas(text) : text;
  ...
  for (let intento = 1; intento <= TTS_MAX_RETRIES; intento++) {
    try {
      ...
      const communicate = new Communicate(textoNarrar, communicateOpts);
      const chunks = [];
