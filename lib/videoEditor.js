...
async function mezclarAmbienteAngelical(videoPath, outputPath, options = {}) {
  const { volumen = 0.28 } = options;
  ...
  const campana = acorde[3] * 2;

  return new Promise((resolve, reject) => {
    logger.info('Mezclando ambiente angelical de fondo', { videoPath, volumen, dur: dur.toFixed(1), acorde });

    const angelicalFilter =
      `sine=frequency=${acorde[0]}:duration=${d}[n1];` +
      `sine=frequency=${acorde[1]}:duration=${d}[n2];` +
      `sine=frequency=${acorde[2]}:duration=${d}[n3];` +
      `sine=frequency=${acorde[3]}:duration=${d}[n4];` +
      `sine=frequency=${campana}:duration=${d}[nc];` +
      '[n1][n2]amix=inputs=2:weights=0.6 0.5[a1];' +
      '[a1][n3]amix=inputs=2:weights=1 0.45[a2];' +
      '[a2][n4]amix=inputs=2:weights=1 0.35[chord];' +
      '[nc]tremolo=f=1.2:d=0.7,volume=0.5[campef];' +
      '[chord][campef]amix=inputs=2:weights=1 0.6[full];' +
      '[full]tremolo=f=0.4:d=0.2[shimmer];' +
      `[shimmer]aecho=0.8:0.9:500|800|1100:0.4|0.3|0.2,volume=${volumen}[ambef];` +
      '[0:a][ambef]amix=inputs=2:duration=first:dropout_transition=0[aout]';

    ffmpeg(videoPath)
      .complexFilter(angelicalFilter)
      .outputOptions(['-map', '0:v', '-map', '[aout]', '-c:v', 'copy', '-c:a', 'aac', '-b:a', '160k', '-shortest'])
      .on('error', (err, stdout, stderr) => {
        const detalle = stderr ? ((err && err.message ? err.message : 'ffmpeg error') + ' | stderr: ' + stderr) : (err && err.message ? err.message : String(err));
        reject(new Error(detalle));
...
module.exports = { getDuration, buildSyncedSegment, buildMultiImageSegment, concatSyncedSegments, burnSubtitles, applyWatermark, mezclarAmbienteTenebroso, mezclarAmbienteAngelical };
