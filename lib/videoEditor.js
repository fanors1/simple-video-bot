...
async function buildSyncedSegment(videoPath, audioPath, outputPath, options = {}) {
  ...
  if (necesitaLoop) {
    ...
    prefijoLoop = `loop=loop=-1:size=${framesClip}:start=0,setpts=N/FRAME_RATE/TB,`;
  }

  const filtroVideo = prefijoLoop + (zoomEntrada ? filtroZoomEscala : filtroBaseEscala);
...
module.exports = { getDuration, buildSyncedSegment, concatSyncedSegments, burnSubtitles, applyWatermark, mezclarAmbienteTenebroso };
