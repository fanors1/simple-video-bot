...
IMPORTANTE sobre como se DICE la cita en la narracion hablada: cuando el narrador mencione la referencia biblica en voz alta, escribila SIEMPRE con palabras, nunca con dos puntos ni numeros pegados. Por ejemplo, en vez de "${v.cita}" se escribe la forma hablada usando la palabra "capitulo" y "versiculo" (ejemplo: "Juan 3:16" se escribe en la narracion como "Juan, capitulo 3, versiculo 16"; "Salmos 23:1" como "Salmos, capitulo 23, versiculo 1"). Esto es porque el motor de voz lee "3:16" como una hora (3 y 16 minutos), lo cual suena mal. En los campos "topic" y "hook_text" SI podes usar el formato corto normal (${v.cita}).
...
async function generateScript({ category, segmentCount = 6, profile: profileName = DEFAULT_PROFILE, account = null } = {}) {
  ...
  if (profileName === 'vive4d') {
    const { elegirVersiculo, leerHistorialVersiculos, guardarVersiculoUsado } = require('./versiculos');
    const canalHist = typeof account === 'string' ? account : (account && account.name) || 'vive4d';
    const histVers = leerHistorialVersiculos(canalHist);
    versiculoElegido = elegirVersiculo(histVers);
    guardarVersiculoUsado(canalHist, versiculoElegido.cita);
    logger.info('Versiculo elegido para VIVE4D', { cita: versiculoElegido.cita, deJesus: versiculoElegido.deJesus, historialPrevio: histVers.length });
  }
...
async function generateImagePostScript({ profile: profileName = DEFAULT_PROFILE, category, account = null } = {}) {
  ...
  const esVive = profileName === 'vive4d';

  let instruccionNicho;
  if (esVive) {
    instruccionNicho = `Eres creador de contenido para el canal cristiano "VIVE4D" (Vive la Palabra), un espacio de fe y esperanza. Genera UN post de imagen de tematica celestial y cristiana de la categoria "${chosenCategory}".
El contenido puede ser: una oracion breve por la familia, un dato o reflexion sobre los angeles, una ensenanza sobre el cielo o la presencia de Dios, una promesa biblica de aliento, o una reflexion espiritual que toque el corazon.
Reglas del canal:
1. Tono calido, esperanzador, reverente y respetuoso. Que transmita paz y cercania con Dios.
2. El titulo debe ser una frase corta y emotiva que invite a la fe (ejemplos de estilo: "SI AMAS A DIOS DEJA UN AMEN", "UNA ORACION POR TU FAMILIA HOY", "LOS ANGELES TE ACOMPANAN SIEMPRE").
3. La descripcion debe conmover e invitar suavemente a interactuar (dejar un amen, compartir, orar), SIN condicionar la bendicion a dar like ni usar formulas de cadena.
4. Respeto absoluto: NO representar a Dios, Jesus ni figuras sagradas en la imagen. Nada de afirmaciones doctrinales polemicas.
5. Si mencionas un versiculo, debe ser real (Reina-Valera 1960) y citado correctamente.`;
  } else if (esHipotesis) {
    instruccionNicho = `Eres creador de contenido viral para el canal "Hipotesis4D", especializado en ESCENARIOS HIPOTETICOS REALISTAS de tipo ucronia ("¿que hubiera pasado si...?").
...
Ejemplos del estilo INCORRECTO (fantasioso, prohibido): "¿Que pasaria si los humanos tuvieran alas?", "¿Y si pudieramos leer la mente?", "¿Que pasaria si existieran los dragones?". NO generes nada asi.`;
    ...
    instruccionNicho = `Eres creador de contenido viral para el canal "Curious4D", especializado en DATOS CURIOSOS reales.
...
El dato debe ser cierto y comprobable, no inventado ni especulativo.`;
  ...
  let ejemploTitulo;
  if (esVive) ejemploTitulo = "SI AMAS A DIOS DEJA UN AMEN";
  else if (esHipotesis) ejemploTitulo = "QUE HUBIERA PASADO SI LOS DINOSAURIOS NUNCA SE HUBIERAN EXTINGUIDO";
  else ejemploTitulo = "EN LO PROFUNDO DEL OCEANO EXISTE UNA CRIATURA QUE NO MUERE";

  const tonoTitulo = esVive ? 'frase emotiva de fe que invita a interactuar' : (esHipotesis ? 'pregunta o premisa hipotetica intrigante' : 'revelacion asombrosa');
  const desarrolloDesc = esVive ? 'la reflexion o mensaje de fe de forma calida y conmovedora, invitando suavemente a dejar un amen o compartir' : (esHipotesis ? 'el escenario hipotetico de forma imaginativa pero anclada en la realidad' : 'el dato de forma fascinante y educativa');
  const cierreDesc = (esVive || esHipotesis) ? '' : ' Incluye la fuente al final si es un hecho cientifico concreto.';
  const imgVive = 'a beautiful serene celestial or spiritual scene matching the message (golden heavenly clouds, rays of divine light, a peaceful church interior with warm light, a family praying in soft golden light, angelic feathers and light, a glowing cross on a hill at sunrise, calm nature with heavenly atmosphere). Warm golden hopeful light, cinematic, photorealistic, deeply peaceful. NEVER depict God, Jesus or sacred figures. No text overlays.';
  const imgOtros = `a striking, dramatic, high-impact image that illustrates ${esHipotesis ? 'the hypothetical scenario' : 'the fact'}. Cinematic, realistic, no text overlays, highly detailed. This is what makes people stop scrolling.`;
...
Responde SOLO con un JSON valido, sin markdown, con este formato exacto:
{
  ...
  "titulo": "titular MUY impactante de 8 a 14 palabras en español, en tono de ${tonoTitulo}, para poner GRANDE sobre la imagen (ej estilo: '${ejemploTitulo}'). Sin comillas internas. Revisa muy bien la ortografia, va a quedar en letras grandes sobre la imagen.",
  "descripcion": "texto de 3 a 5 parrafos cortos en español que desarrollan ${desarrolloDesc}, para el pie de foto. Termina con una pregunta o invitacion que motive a comentar.${cierreDesc}",
  "image_prompt": "detailed photorealistic image description in ENGLISH for ${esVive ? imgVive : imgOtros}",
